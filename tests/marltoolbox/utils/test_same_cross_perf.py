...
def test__get_config_for_one_same_play():
    ...
    def assert_(checkpoint_i):
        ...
        assert config_copy["multiagent"]["policies"][own_policy_id][3][restore.LOAD_FROM_CONFIG_KEY][0] == \
            evaluator.checkpoints[checkpoint_i]["path"]

        opp_policy_id = evaluator.policies_to_load_from_checkpoint[1]
        assert metadata[opp_policy_id]["checkpoint_path"] == evaluator.checkpoints[checkpoint_i]["path"]
        assert metadata[opp_policy_id]["checkpoint_i"] == checkpoint_i
        assert config_copy["multiagent"]["policies"][opp_policy_id][3][restore.LOAD_FROM_CONFIG_KEY][0] == \
            evaluator.checkpoints[checkpoint_i]["path"]
...
def test__get_config_for_one_cross_play():
    ...
    def assert_(checkpoint_i):
        n_cross_play_per_checkpoint = train_n_replicates - 1
        opponents_idx_per_checkpoint = evaluator._get_opponents_per_checkpoints(n_cross_play_per_checkpoint)
        ...
        for cross_play_i in range(n_cross_play_per_checkpoint):
            metadata, config_copy = evaluator._get_config_for_one_cross_play(
                checkpoint_i,
                opponent_i=opponents_idx[cross_play_i])
            ...
            opponent_checkpoint = evaluator.checkpoints[opponents_idx[cross_play_i]]
            opp_policy_id = evaluator.policies_to_load_from_checkpoint[1]
            assert metadata[opp_policy_id]["checkpoint_path"] == opponent_checkpoint["path"]
            assert metadata[opp_policy_id]["checkpoint_i"] == opponents_idx[cross_play_i]
            assert config_copy["multiagent"]["policies"][opp_policy_id][3][restore.LOAD_FROM_CONFIG_KEY][0] == \
