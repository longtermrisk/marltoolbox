...
def test_logged_info_no_picking():
    ...
    for env in [coin_game, asymm_coin_game]:
        ...
        assert_info(batch_deltas, n_steps, batch_size, p_red_act, p_blue_act, env, grid_size, max_steps,
                    p_red_pos, p_blue_pos, c_red_pos, c_blue_pos,
...
def test_observations_are_invariant_to_the_player_trained():
    ...
    for env_i, env in enumerate([coin_game, asymm_coin_game]):
        ...
        for _ in range(n_steps):
            overwrite_pos(step_i, batch_deltas, max_steps, env, p_red_pos, p_blue_pos,
