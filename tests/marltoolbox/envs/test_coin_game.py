...
def assert_info(n_steps, batch_size, p_red_act, p_blue_act, env, grid_size, max_steps,
    ...
    for _ in range(n_steps):
        ...
        if done["__all__"]:
            ...
            if red_own is None:
                assert "pick_own" not in info["player_red"]
            else:
                assert info["player_red"]["pick_own"] == red_own
            if blue_own is None:
                assert "pick_own" not in info["player_blue"]
            else:
                assert info["player_blue"]["pick_own"] == blue_own
