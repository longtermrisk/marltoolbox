...
from ray.rllib.agents.dqn.dqn_torch_policy import \
    DQNTorchPolicy, build_q_stats, after_init
...
def main(debug):
    ...
    lvl0_seeds = [seed + timestamp
                  for seed in list(range(n_in_lvl0_population))]
    ...
    if tune_hparams["load_data"] is None:
        ...
        log.pprint_saved_metrics(
            tune_hparams["load_data"],
             keywords_to_print=["policy_reward_mean", "speed.*mean",
                                "own.*mean", "analysis", "^avg$",
                                "last-10-avg"])
...
def get_tune_config(hp: dict):
    ...
    if tune_config['env'] in (VectorizedCoinGame, AsymVectorizedCoinGame):
        ...
        tune_config['batch_size'] = \
            4000 if hp['batch_size'] is None else hp['batch_size']
        tune_config['lr'] = \
            1.0 if hp['lr'] is None else hp['lr']
        tune_config['gamma'] = \
            0.96 if hp['gamma'] is None else hp['gamma']
        hp["x_limits"] = ((-3.0, 3.0),)
        hp["y_limits"] = ((-3.0, 3.0),)
        hp["jitter"] = 0.05
...
def train_lvl0_population(tune_hp):
    ...
    return tune.run(tune_hp["tune_class"],
    ...
    if tune_hp["env"] == IteratedPrisonersDilemma:
        ...
        rllib_hp["temperature_schedule"] = PiecewiseSchedule(
            endpoints=[
                (0,
                 2.0),
                (int(rllib_hp["n_steps_per_epi"] * rllib_hp["n_epi"] * 0.50),
                 0.1)],
            outside_value=0.1,
            framework="torch")
...
def get_rllib_config(hp: dict, lvl1_idx: list, lvl1_training: bool):
    ...
    def sgd_optimizer_dqn(policy, config) -> "torch.optim.Optimizer":
        return torch.optim.SGD(policy.q_func_vars,
                               lr=policy.cur_lr,
    ...
    rllib_config = {
        ...
        # "seed": hp["seed"],
        "seed": tune.grid_search(hp["lvl1_seeds"]
                                 if lvl1_training else hp["lvl0_seeds"]),
