...
from marltoolbox.algos.alternating_offers.envs.alt_offers_env import (
    AltOffersEnv,
)
from marltoolbox.experiments.tune_class_api.alternating_offers import (
    train_cross_play,
)
...
matches = {}
matches["custom_vs_custom_1"] = [
    trial
    for trial in trial_info
    if (trial["params"]["match_mode"] == "custom_vs_custom_1")
]
matches["custom_vs_custom_2"] = [
    trial
    for trial in trial_info
    if (trial["params"]["match_mode"] == "custom_vs_custom_2")
]
matches["default_vs_custom"] = [
    trial
    for trial in trial_info
    if (trial["params"]["match_mode"] == "default_vs_custom")
]
matches["custom_vs_default"] = [
    trial
    for trial in trial_info
    if (trial["params"]["match_mode"] == "custom_vs_default")
]
matches["default_vs_default"] = [
    trial_info[default_i],
...
for match_regime, match_trials in matches.items():
    if match_regime == "default_vs_default":
        match_indices["default_vs_default"] = range(
            len(matches["default_vs_default"])
        )
    else:
        for cond_params_i, cond_params in enumerate(cond_params_list):
            eq_list = [
                ...
                if trial["params"]["cond_params"] == cond_params
            ]
            ind = eq_list[0][0]
            match_indices[match_regime].append(ind)

results_filename = os.path.join(
    "results", "results_" + "_".join(root_path.split("/")[-3:]) + ".pickle"
)
    ...
    for cond_params_i in trange(len(cond_params_list)):
        ...
        config = {
            ...
            "hidden_embedding_sizes": [
                agent_0_params["hidden_embedding_size"],
                agent_1_params["hidden_embedding_size"],
            ],
            "utility_types": [
                f"skew_{agent_0_params['utilities_skew']:.1f}",
                f"skew_{agent_1_params['utilities_skew']:.1f}",
            ],
            "prosociality_levels": [
                agent_0_params["prosociality_level"],
                agent_1_params["prosociality_level"],
            ],
            "fairness_coeffs": [
                agent_0_params["fairness_coeff"],
                ...
                if reward_name not in all_rewards:
                    ...
                    all_rewards[reward_name] = np.concatenate(
                        [
                            ...
                            rewards[reward_name].numpy(),
                        ]
                    )
        results[(agent_0_match_regime, agent_1_match_regime)].append(
            all_rewards
        )
