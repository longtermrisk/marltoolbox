...
def train_lvl0_population(hp):
    ...
    lvl0_tune_analysis_per_welfare = (
        amtft_various_env.train_for_each_welfare_function(hp)
    )
...
def train_lvl1_agents(hp_lvl1, tune_analysis_lvl0):
    print("==================================================================")
    print(
        "Start training level 1 agents against populations "
        "of frozen level 0 amTFT agents"
    )
    ...
    tune_analysis_lvl1 = ray.tune.run(
        dqn.DQNTrainer,
        config=rllib_config,
        stop=stop,
...
def modify_conf_for_lvl1_training(
    hp_lvl1, env_config, rllib_config_lvl1, lvl0_checkpoints
):
    lvl0_policy_idx = 1
    lvl1_policy_idx = 0
