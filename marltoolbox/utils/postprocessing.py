...
# TODO remove one of the 2 ways
# TODO transform that in a function for postprocess_trajectory of a policy
#   why? because this way we can select which policy add which stuff instead of doing it for everyone?
# def get_WelfareAndPostprocessCallbacks(
...
#         add_advantages=False):
#     class WelfareAndPostprocessCallbacks(DefaultCallbacks):
#
#         def on_episode_end(self, *, worker: "RolloutWorker", base_env: BaseEnv,
#                            policies: Dict[PolicyID, Policy],
#                            episode: MultiAgentEpisode, env_index: int, **kwargs):
#
#             if add_egalitarian_welfare or add_nash_welfare:
...
#             # if add_advantages:
#             #     postprocessed_batch = add_advantages_fn(policy=policies[policy_id], sample_batch=postprocessed_batch)
#
...
#             print("all_batchs_rewards", all_batchs_rewards)
#             print("util", np.array([sum(reward_points) for reward_points in zip(*all_batchs_rewards)]))
#             return sample_batch
#
#         @staticmethod
...
#             # print("sample_batch.REWARDS", len(sample_batch[sample_batch.REWARDS]), sample_batch[sample_batch.REWARDS])
#             # print("welfare", len(welfare), welfare)
#             sample_batch.data[WELFARE_INEQUITY_AVERSION] = welfare.tolist()
#             print("inequity inversion", welfare)
#
#             return sample_batch
#
#         @staticmethod
...
#             return sample_batch
#
#     return WelfareAndPostprocessCallbacks
...
def _add_utilitarian_welfare_to_batch(sample_batch: SampleBatch,
    ...
    # print("own_rewards", sample_batch[sample_batch.REWARDS])
    # print("opp_rewards", opp_ag_batchs[0][opp_ag_batchs[0].REWARDS])
    # print("util", np.array([sum(reward_points) for reward_points in zip(*all_batchs_rewards)]))
    return sample_batch
...
def _add_inequity_aversion_welfare_to_batch(sample_batch: SampleBatch, opp_ag_batch: SampleBatch,
    ...
    own_rewards = np.array(sample_batch[sample_batch.REWARDS])
    opp_rewards = np.array(opp_ag_batch[opp_ag_batch.REWARDS])
    # print("own_rewards", own_rewards)
    # print("opp_rewards", opp_rewards)
    own_rewards = np.flip(own_rewards)
    opp_rewards = np.flip(opp_rewards)
    delta = (discount(own_rewards, gamma * lambda_) - discount(opp_rewards, gamma * lambda_))
    delta = np.flip(delta)
