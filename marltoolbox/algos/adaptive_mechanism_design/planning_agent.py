...
logging.basicConfig(
    filename="Planning_Agent.log", level=logging.DEBUG, filemode="w"
)
from marltoolbox.algos.adaptive_mechanism_design.agent import (
    Agent,
    convert_from_rllib_env_format,
)
...
class Planning_Agent(Agent):
    def __init__(
        ...
        with tf.variable_scope("Planner"):
            self.s = tf.placeholder(
                tf.float32, [1, env.n_features], "state_pl"
            )
            self.a_players = tf.placeholder(
                tf.float32, [1, n_players], "player_actions"
            )
            ...
            if value_fn_variant == "exact":
                if self.convert_a_to_one_hot:
                    self.p_players = tf.placeholder(
                        tf.float32,
                        [1, n_players, env.NUM_ACTIONS],
                        "player_action_probs",
                    )
                else:
                    self.p_players = tf.placeholder(
                        tf.float32, [1, n_players], "player_action_probs"
                    )
                self.a_plan = tf.placeholder(
                    tf.float32, [2, 2], "conditional_planning_actions"
                )  # works only for matrix games
            self.r_players = tf.placeholder(
                tf.float32, [1, n_players], "player_rewards"
            )

            if self.convert_a_to_one_hot:
                self.inputs = tf.concat(
                    [self.s, self.a_players_one_hot_reshape], 1
                )
            else:
            ...
            with tf.variable_scope("Policy_p"):
                ...
                if not isinstance(n_units, list):
                    units = [
                        env.n_features + ma_action_space_dim,
                        n_units,
                        n_players,
                    ]
                else:
            ...
            with tf.variable_scope("Vp"):
                if max_reward_strength is not None:
                    self.vp = (
                        2 * max_reward_strength * (self.action_layer - 0.5)
                    )
                else:
            ...
            with tf.variable_scope("cost_function"):
                ...
                for underlying_agent in underlying_agents:
                    ...
                    if value_fn_variant == "exact":
                        ...
                        if self.convert_a_to_one_hot:
                            self.g_p = self.p_players[0, idx, act_idx] * (
                                1 - self.p_players[0, idx, act_idx]
                            )
                            self.p_opp = self.p_players[0, 1 - idx, act_idx]
                            grad = tf.gradients(
                                ys=self.vp[0, idx], xs=self.a_players
                            )
                            if add_state_grad:
                                grad_s = tf.gradients(
                                    ys=self.vp[0, idx], xs=self.s
                                )
                            self.g_Vp = self.g_p * grad[0][0, idx]
                            if add_state_grad:
                                self.g_Vp += self.g_p * tf.reduce_sum(grad_s)
                        else:
                            self.g_p = self.p_players[0, idx] * (
                                1 - self.p_players[0, idx]
                            )
                        ...
                        if "CoinGame" in self.env_name:
                            ...
                            if not use_v_pl:
                                self.g_V = self.g_p * (
                                    self.p_opp * (2 * env.R - env.T - env.S)
                                    + (1 - self.p_opp)
                                    * (env.T + env.S - 2 * env.P)
                                )
                            else:
                    ...
                    cost_list.append(
                        -underlying_agent.learning_rate * self.g_Vp * self.g_V
                    )
                ...
                if with_redistribution:
                    if square_cost:
                        ...
                        self.extra_loss = cost_param * tf.norm(
                            self.vp - tf.reduce_mean(self.vp)
                        )
                else:
                    if square_cost:
                        self.extra_loss = (
                            cost_param * tf.norm(self.vp) * tf.norm(self.vp)
                        )
                    else:
                ...
                self.dynamic_scaling_vp(
                    normalize_against_vp,
                    max_reward_strength,
                    normalize_vp_separated,
                )
                self.dynamic_scaling_v(normalize_against_v)

                if planner_clip_norm is not None:
                    self.cost = tf.clip_by_norm(
                        self.cost, planner_clip_norm, axes=None, name=None
                    )
            ...
            with tf.variable_scope("trainPlanningAgent"):
                ...
                if use_adam_optimizer:
                    self.train_op = tf.train.AdamOptimizer(
                        self.loss_mul_planner * learning_rate
                    ).minimize(
                        self.loss,
    ...
    def convertion_to_one_hot(self, use_softmax_hot):
        ...
        self.a_players_one_hot_reshape = tf.reshape(
            self.a_players_one_hot, (1, -1)
        )
    ...
    def create_multi_layer_fc(self, units, mean_theta, std_theta):
        ...
        self.w_pi0 = tf.Variable(
            tf.random_normal([n_in, n_out], mean=0.0, stddev=std_theta)
        )
        self.b_pi0 = tf.Variable(
    ...
    def dynamic_scaling_vp(
        self, normalize_against_vp, max_reward_strength, normalize_vp_separated
    ):
        if "CoinGame" in self.env_name:
            init_v = 0.1
        else:
        ...
        if not normalize_vp_separated:
            ...
            self.mean_vp_in = tf.placeholder(
                tf.float32, shape=(), name="mean_vp"
            )
        else:
            temp_v = init_v * normalize_against_vp
            self.mean_vp_np = [temp_v, temp_v]
            self.mean_vp_in = tf.placeholder(
                tf.float32, shape=(2,), name="mean_vp"
            )
        ...
        if normalize_against_vp:
            if not normalize_vp_separated:
                null = 0.0
                self.mean_vp_out = (
                    1 - (1 / normalize_against_vp)
                ) * self.mean_vp_in + tf.math.abs(tf.reduce_sum(self.vp))
                self.cost = tf.cond(
                    tf.equal(self.mean_vp_out, null),
                    lambda: null,
                    lambda: self.cost
                    / (
                        self.mean_vp_out
                ...
                self.mean_vp_out = (
                    1 - (1 / normalize_against_vp)
                ) * self.mean_vp_in + tf.math.abs(
                    tf.reduce_sum(self.vp, axis=0)
                )
                cost_list = []
                cost_list.append(
                    tf.cond(
                        tf.equal(self.mean_vp_out[0], 0.0),
                        lambda: null,
                        lambda: self.cost[0]
                        / (
                            self.mean_vp_out[0]
                ...
                cost_list.append(
                    tf.cond(
                        tf.equal(self.mean_vp_out[1], 0.0),
                        lambda: null,
                        lambda: self.cost[1]
                        / (
                            self.mean_vp_out[1]
    ...
    def dynamic_scaling_v(self, normalize_against_v):
        ...
        if normalize_against_v:
            self.mean_v_out = (
                1 - (1 / normalize_against_v)
            ) * self.mean_v_in + tf.reduce_sum(tf.math.abs(self.v))
            self.cost = tf.cond(
                tf.equal(self.mean_v_out, 0.0),
                lambda: 0.0,
                lambda: self.cost / (self.mean_v_out / normalize_against_v),
            )
        else:
    ...
    def learn(self, s, a_players, coin_game=False, env_rewards=None):
        ...
        if env_rewards is None:
            if coin_game:
                ...
                actions = {
                    "player_red": a_players[0],
                    "player_blue": a_players[1],
                }
                r_players_rllib_format = self.env._compute_rewards(s, actions)
            else:
                r_players_rllib_format = self.env._compute_rewards(*a_players)
            r_players = convert_from_rllib_env_format(
                r_players_rllib_format, self.env.players_ids
            )
        else:
        ...
        if self.value_fn_variant == "estimated":
            ...
            for underlying_agent in self.underlying_agents:
                idx = underlying_agent.agent_idx
                # if "CoinGame" in self.env_name:
                g_log_pi_list.append(
                    underlying_agent.calc_g_log_pi(s, a_players_one_hot[idx])[
                        0
                    ][0, ...]
                )
                # else:
        ...
        if self.value_fn_variant == "exact":
            ...
            for underlying_agent in self.underlying_agents:
                idx = underlying_agent.agent_idx
                if self.convert_a_to_one_hot:
                    p_players_list.append(
                        underlying_agent.calc_action_probs(s, add_dim=False)
                    )
                else:
                    p_players_list.append(
                        underlying_agent.calc_action_probs(s)[0, -1]
                    )  # Only 2 actions
                # if "CoinGame" in self.env_name:
    ...
    def choose_action(self, s, a_players):
        ...
        a_plan = self.sess.run(
            self.vp, {self.s: s, self.a_players: a_players[np.newaxis, ...]}
        )[0, :]
    ...
    def calc_conditional_planning_actions(self, s):
        ...
        a_plan_DD = self.sess.run(
            self.action_layer,
            {self.s: s, self.a_players: np.array([0, 0])[np.newaxis, :]},
        )
        a_plan_CD = self.sess.run(
            self.action_layer,
            {self.s: s, self.a_players: np.array([1, 0])[np.newaxis, :]},
        )
        a_plan_DC = self.sess.run(
            self.action_layer,
            {self.s: s, self.a_players: np.array([0, 1])[np.newaxis, :]},
        )
        a_plan_CC = self.sess.run(
            self.action_layer,
            {self.s: s, self.a_players: np.array([1, 1])[np.newaxis, :]},
        )
        l_temp = [a_plan_DD, a_plan_CD, a_plan_DC, a_plan_CC]
        if self.max_reward_strength is not None:
            l = [
                2 * self.max_reward_strength * (a_plan_X[0, 0] - 0.5)
                for a_plan_X in l_temp
            ]
        else:
        ...
        if self.with_redistribution:
            if self.max_reward_strength is not None:
                l2 = [
                    2 * self.max_reward_strength * (a_plan_X[0, 1] - 0.5)
                    for a_plan_X in l_temp
                ]
            else:
