        ...
        agent_config.update(
            {
                ...
                # Learning rate
                "lr": lr * env.max_steps,
                # Training every episode instead of every step
            ...
            with tf.variable_scope(f"player_{i}"):
                policy = A3CTFPolicy(
                    obs_space=gym.spaces.Box(
                        low=0, high=1, shape=(env.n_features,), dtype="uint8"
                    ),
...
class AdaptiveMechanismDesign(tune.Trainable):
    def _init_algo(
        ...
        seed,
        normalize_planner,
        no_weights_decay_planner,
        planner_std_theta_mul,
        use_adam_optimizer,
        use_softmax_hot,
        report_every_n,
        momentum,
        ...
        **kwargs,
        ...
        if env == "FearGreedMatrix":
            env = define_greed_fear_matrix_game(fear=fear, greed=greed)(
                env_config
            )
        elif env == "CoinGame":
        ...
        if self.use_rllib_polcy:
            self.multi_agent_batch_builder = MultiAgentSampleBatchBuilder(
                policy_map={
                    idx: player for idx, player in enumerate(self.players)
                },
                clip_rewards=False,
    ...
    def step(self):
        ...
        for _ in range(self.report_every_n):
            ...
            while not done:
                ...
                (
                    s_rllib_format,
                    rewards_rllib_format,
                    done_rllib_format,
                    info_rllib_format,
                ) = self.env.step(actions_rllib_format)
                ...
                done = convert_from_rllib_env_format(
                    done_rllib_format, self.player_ids
                )
                ...
                if (
                    ...
                    and self.epi_n < self.n_planning_eps
                ):
                ...
                for idx, player in enumerate(self.players):
                    if self.use_rllib_polcy:
                        step_player_values = {
                            ...
                            "dones": done,
                            "vf_preds": player._value(
                                obs_before_act, prev_a[idx], prev_r[idx]
                            ),
                        }
                ...
                if self.use_rllib_polcy and done:
                    multiagent_batch = (
                        self.multi_agent_batch_builder.build_and_reset()
                    )
                    for idx, player in enumerate(self.players):
    ...
    def _add_info_to_log(
        ...
        for idx, player in enumerate(self.players):
            ...
            if planning_reward_when_pick_own_coin[idx] is not None:
                to_report[
                    f"pl_rw_p{idx}_pick_own_coin"
                ] = planning_reward_when_pick_own_coin[idx]
            if planning_reward_when_pick_opp_coin[idx] is not None:
                to_report[
                    f"pl_rw_p{idx}_pick_opp_coin"
                ] = planning_reward_when_pick_opp_coin[idx]
            if planning_reward_when_no_picking[idx] is not None:
                to_report[
                    f"pl_rw_p{idx}_no_picking"
                ] = planning_reward_when_no_picking[idx]

            for act_v in range(self.env.NUM_ACTIONS):
                if (
                    planning_reward_when_specific_action[idx][act_v]
                    is not None
                ):
                    to_report[
                        f"pl_rw_p{idx}_a{act_v}"
                    ] = planning_reward_when_specific_action[idx][act_v]
