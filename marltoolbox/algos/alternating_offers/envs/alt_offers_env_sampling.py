...
def sample_utility(
    batch_size, utility_type, num_values=6, num_items=3, random_state=np.random
):
    ...
    while u.sum() == 0:
        if utility_type == "uniform":
            ...
            item_skews = [
                (item_i - zero_skew_item_i) / zero_skew_item_i * skew_val
            ...
            for item_i in range(num_items):
                ...
                value_skews = [
                    (value_i - zero_skew_value_i)
                    / zero_skew_value_i
                    * item_skews[item_i]
                    for value_i in range(num_values)
                ]
                cur_probs = [
                    1 / num_values * (1 + value_skews[value_i])
                    for value_i in range(num_values)
                ]
...
def generate_batch(batch_size, utility_types, random_state=np.random):
    ...
    utilities.append(
        sample_utility(
            ...
            num_items=3,
            random_state=random_state,
            utility_type=utility_types[1],
        )
    )
    N = sample_N(batch_size=batch_size, random_state=random_state)
    return {"pool": pool, "utilities": utilities, "N": N}
