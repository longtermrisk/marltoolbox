...
def sample_utility(batch_size, utility_type, num_values=6, seq_len=3, random_state=np.random):
    ...
    while u.sum() == 0:
        if utility_type == 'uniform':
            ...
            u = torch.from_numpy(random_state.choice([1, 5], (batch_size, seq_len), replace=True))
        elif utility_type == '3_4_5_only':
            u = torch.from_numpy(random_state.choice([3, 4, 5], (batch_size, seq_len), replace=True))
        elif utility_type == 'max_on_0':
            u = random_state.choice(num_values, (batch_size, seq_len), replace=True)
            u[:, 0] = 5
...
def generate_batch(batch_size, utility_type, random_state=np.random):
    ...
    utilities.append(sample_utility(batch_size=batch_size, num_values=6, seq_len=3, random_state=random_state, utility_type=utility_types[0]))
    utilities.append(sample_utility(batch_size=batch_size, num_values=6, seq_len=3, random_state=random_state, utility_type=utility_types[1]))
