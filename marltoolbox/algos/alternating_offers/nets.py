...
class NumberSequenceEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(
            0, 1
        )  # now 0 is sequence dim, 1 is batch dim - they've been transposed
        x = self.embedding(x)
        ...
        for s in range(seq_len):
            state = self.lstm(
                x[s], state
            )  # takes the whole batch but only at embedded timestep s
...
class TermPolicy(nn.Module):
    ...
    def forward(self, thoughtvector, testing):
        ...
        entropy = -(
            response_probs * response_probs.log()
            + (1 - response_probs) * (1 - response_probs).log()
        ).sum()
        # for the batch:
    ...
    def forward(self, x, testing):
        ...
        for i in range(self.num_items):
            ...
            if not testing:
                ...
                g = torch.gather(
                    probs, 1, Variable(a.data)
                )  # place probs on indices specified in samples
                log_g = g.log()
                a = a.data
            else:
                a = res_greedy
...
class ProposalReprPolicy(nn.Module):
    ...
    def forward(self, x, hidden_proposal, testing):
        ...
        type_constr = torch.cuda if x.is_cuda else torch
        stochastic_draws = 0
        hidden_proposal_onehot = torch.zeros(
            batch_size, self.num_items, self.num_counts
        )
        if x.is_cuda:
            hidden_proposal_onehot = hidden_proposal_onehot.cuda()
        hidden_proposal_onehot.scatter_(
            2, hidden_proposal.unsqueeze(2), 1
        )  # dim, index, src val
        proposal = type_constr.LongTensor(batch_size, self.num_items).fill_(
            0
        )  # new public proposal
...
class RedistributionPolicy(nn.Module):
    ...
    def forward(self, thoughtvector, testing, mid_move_indices):
        ...
        favor_probs = torch.clamp(
            F.sigmoid(self.favor_first_player(thoughtvector)), eps, 1 - eps
        )

        enable_decision = torch.bernoulli(
            enable_probs
        ).long()  # sample a decision for the batch elems
        enable_argmax = enable_decision == (enable_probs > 0.5).long()
        enable_logprob = (
            enable_probs * enable_decision.float().detach()
            + (1 - enable_probs) * (1 - enable_decision.float().detach())
        ).log()
        enable_entropy = (
            -enable_probs * enable_probs.log()
            - (1 - enable_probs) * (1 - enable_probs).log()
        )
        ...
        favor_logprob = (
            favor_probs * favor_decision.float().detach()
            + (1 - favor_probs) * (1 - favor_decision.float().detach())
        ).log()
        favor_entropy = (
            -favor_probs * favor_probs.log()
            - (1 - favor_probs) * (1 - favor_probs).log()
        )
        ...
        return (
            enable_logprob,
            enable_probs_clone,
            enable_decision.data.byte(),
            enable_entropy,
            favor_logprob,
            favor_probs_clone,
            favor_decision.data.byte(),
            favor_entropy,
            enable_argmax_count,
            favor_argmax_count,
            thoughtvector.size(0) - len(mid_move_indices),
            enable_decision.sum().item(),
        )
...
class AgentModel(nn.Module):
    def __init__(
        ...
        self.lstm = nn.LSTMCell(
            input_size=hidden_embedding_size, hidden_size=hidden_embedding_size
        )

        self.combined_net = nn.Sequential(
            nn.Linear(19, hidden_embedding_size), nn.ReLU()
        )
    ...
    def forward(
        ...
        timestep_formatted = np.reshape(
            np.repeat(np.array([timestep]), batch_size), (batch_size, 1)
        )
        ...
        h_t = torch.cat(
            ...
            -1,
        )  # (static game context, utterance, proposal)
        h_t = self.combined_net(
            h_t
        )  # act on (static game context, utterance, proposal) with linear and relu

        hidden_state, cell_state = self.lstm(
            h_t, (hidden_state, cell_state)
        )  # takes the whole batch but only at embedded timestep s
        h_t = hidden_state
        ...
        (
            ...
            response,
            response_entropy,
            response_matches_argmax_count,
            forward_stats["response_stochastic_draws"],
        ) = self.response_policy(h_t, testing=deterministic)
        forward_stats["response_prob"] = response_probs.sum().item()
        forward_stats["response_entropy"] = response_entropy.item()
        forward_stats[
            "response_matches_argmax_count"
        ] = response_matches_argmax_count.sum().item()
        nodes.append(response_node)
        entropy_loss -= (
            self.response_entropy_reg * response_entropy
        )  # maximize entropy so minimize loss ~ (-entropy)
        ...
        (
            ...
            forward_stats["prop_stochastic_draws"],
        ) = self.proposal_policy(h_t, testing=deterministic)
        forward_stats["prop_entropy"] = prop_entropy.item()
        forward_stats[
            "prop_matches_argmax_count"
        ] = prop_matches_argmax_count.sum().item()
        ...
        if self.enable_cheap_comm:
            (
                ...
                forward_stats["utt_stochastic_draws"],
            ) = self.proposal_repr_policy(h_t, proposal, testing=deterministic)
            forward_stats["utt_entropy"] = utt_entropy.item()
            forward_stats[
                "utt_matches_argmax_count"
            ] = utt_matches_argmax_count.sum().item()
...
class ArbitratorModel(nn.Module):
    def __init__(
        ...
        print(input_size)
        self.combined_net = nn.Sequential(
            nn.Linear(input_size, hidden_embedding_size), nn.ReLU()
        )
    ...
    def forward(
        ...
        prop_encoded = (
            proposal.clone()
        )  # encoding final proposal with same net if it's non-zero
        ...
        h_t = torch.cat(
            [ten.float() for ten in input_tens], -1
        )  # (static game context, utterance, proposal)
        h_t = self.combined_net(
            h_t
        )  # act on (static game context, utterance, proposal) with linear and relu
        ...
        (
            ...
            favor_decision,
            favor_entropy,
            forward_stats["enable_argmax_count"],
            forward_stats["favor_argmax_count"],
            forward_stats["enable_draws"],
            forward_stats["favor_draws"],
        ) = self.redist_policy(
