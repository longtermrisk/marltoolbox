        ...
        for agent_i in range(2):
            if isinstance(args["hidden_embedding_sizes"][agent_i], list):
                ...
                if args["agents_sgd"]:
                    ...
                    cur_pop_opts = [
                        optim.Adam(params=model.parameters()) for model in cur_pop
                    ]
                ...
                if self.fixed_agent_i is not None and agent_i != self.fixed_agent_i:
                    if args["agents_sgd"]:
                        ...
                        self.agent_opts.append(
                            optim.Adam(params=self.agent_models[agent_i].parameters())
                        )
                else:
        ...
        if args["enable_arbitrator"]:
            ...
            if args["arbitrator_sgd"]:
                self.arbitrator_opt = optim.SGD(
                    self.arbitrator_model.parameters(), lr=1e-3, weight_decay=1e-4
                )
            else:
                ...
                for (
                    ...
                    for action in action_logprobs[
                        ...
                        if index_0 is not None:
                            ...
                            cur_prosociality_level = self.args["prosociality_levels"][
                                agent
                            ][pop_i]
