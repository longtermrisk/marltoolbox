...
from marltoolbox.algos.alternating_offers.envs.alt_offers_env_sieve import (
    SievePlayback,
)
...
class AltOffersTraining(tune.Trainable):
    def setup(self, args):
        ...
        for i in range(2):
            ...
            if args["agents_sgd"]:
                self.agent_opts.append(
                    torch.optim.SGD(
                        self.agent_models[i].parameters(),
                        lr=1e-3,
                        weight_decay=1e-4,
                    )
                )
                ...
                self.agent_opts.append(
                    optim.Adam(params=self.agent_models[i].parameters())
                )
        ...
        if args["enable_arbitrator"]:
            ...
            if args["arbitrator_sgd"]:
                self.arbitrator_opt = torch.optim.SGD(
    ...
    def step(self):
        ...
        for cur_episode_i in range(self.args["episodes_per_step"]):
            ...
            baselined_rewards = {
                name: rewards[name] - self.baseline[name] for name in rewards
            }
            ...
            if self.args["enable_arbitrator"]:
                ...
                for (
                    ...
                    for action in action_logprobs[
                        ...
                        orig_welfare_reward = (
                            baselined_rewards["sum_share_of_max"][global_idxes]
                            .float()
                            .contiguous()
                ...
                for i in range(2):
                    ...
                    cur_grads = torch.autograd.grad(
                        induced_reward_by_agent[i] - entropy_loss_by_agent[i],
                        self.agent_models[i].parameters(),
                        retain_graph=True,
                        create_graph=True,
                    )
            ...
            for (
                ...
                for action in action_logprobs[
                    ...
                    assert (
                        ...
                        or self.args["prosociality_levels"][agent] == 0
                    )
                    ...
                    assert (
                        ...
                        or self.args["fairness_coeffs"][agent] == 0
                    )
                    total_agent_reward = (
                        (1 - self.args["prosociality_levels"][agent])
                        * baselined_rewards[reward_name][global_idxes]
                        + self.args["prosociality_levels"][agent]
                        * baselined_rewards["sum_share_of_max"][global_idxes]
                        - self.args["fairness_coeffs"][agent]
            ...
            for i in range(2):
                ...
                self.entropy_loss_by_agent_accum[i] += entropy_loss_by_agent[
                    i
                ].item()
    ...
    def load_checkpoint(self, tmp_checkpoint_dir):
        ...
        for i in range(2):
            self.agent_models[i].load_state_dict(
                state["agent%s" % i]["model_state"]
            )
            self.agent_opts[i].load_state_dict(
...
def run_episode(
    args, env_state, agent_models, arbitrator_model, deterministic, render_flag
):
    env_agents_state = AltOffersEnvMemory(
        env_state,
        msg_len=3,
        hidden_embedding_sizes=args["hidden_embedding_sizes"],
