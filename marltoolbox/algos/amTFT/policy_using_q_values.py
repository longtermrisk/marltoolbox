from marltoolbox.algos.amTFT.base import \
    OPP_SELFISH_POLICY_IDX, OWN_SELFISH_POLICY_IDX
...
class amTFTQValuesTorchPolicy(AmTFTPolicyBase):
    ...
    def _compute_debit(self, last_obs, opp_action, worker, base_env, episode,
        ...
        approximated_debit = self._compute_debit_from_q_values(last_obs,
                                                               opp_action,
                                                               coop_opp_simulated_action)
        return approximated_debit

    def _compute_debit_from_q_values(self, last_obs, opp_action,
                                     coop_opp_simulated_action):
        ...
        temperature_used_for_exploration = self.algorithms[
            OWN_SELFISH_POLICY_IDX].exploration.temperature
        ...
        debit = (self.selfish_opp_q_values[
                     0, opp_action] * temperature_used_for_exploration -
                 self.selfish_opp_q_values[
                     0, coop_opp_simulated_action] * temperature_used_for_exploration)
        self._to_log['raw_debit'] = debit
        return debit

    def _compute_selfish_opp_q_values(self, last_obs):
        _, _, selfish_opp_extra_fetches = self.algorithms[
            OPP_SELFISH_POLICY_IDX].compute_actions([last_obs])
    ...
    def _compute_punishment_duration(self, opp_action,
                                     coop_opp_simulated_action, worker,
                                     last_obs):
        return self._compute_punishment_duration_from_q_values(opp_action,
                                                               coop_opp_simulated_action)

    def _compute_punishment_duration_from_q_values(self, opp_action,
                                                   coop_opp_simulated_action):
        ...
        opp_expected_lost_per_step = q_coop[coop_opp_simulated_action] / 2 - \
                                     q_selfish[opp_action]
        print("self.total_debit", self.total_debit)
        print("opp_expected_lost_per_step", opp_expected_lost_per_step)
        n_steps_equivalent = (
                                         self.total_debit * self.punishment_multiplier) / opp_expected_lost_per_step
