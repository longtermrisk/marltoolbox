...
class amTFTQValuesTorchPolicy(amTFTPolicyBase):
    ...
    def _compute_debit(self, last_obs, opp_action, worker, base_env, episode, env_index, coop_opp_simulated_action):
        approximated_debit = self._compute_debit_from_q_values(last_obs, opp_action, coop_opp_simulated_action)
        return approximated_debit

    def _compute_debit_from_q_values(self, last_obs, opp_action, coop_opp_simulated_action):
        self.selfish_opp_q_values = self._compute_selfish_opp_q_values(last_obs)
    ...
    def _compute_selfish_opp_q_values(self, last_obs):
        _, _, selfish_opp_extra_fetches = self.algorithms[OPP_SELFISH_POLICY_IDX].compute_actions([last_obs])
    ...
    def _compute_punishment_duration(self, opp_action, coop_opp_simulated_action, worker, last_obs):
        return self._compute_punishment_duration_from_q_values(opp_action, coop_opp_simulated_action)

    def _compute_punishment_duration_from_q_values(self, opp_action, coop_opp_simulated_action):
