...
class AmTFTRolloutsTorchPolicy(AmTFTPolicyBase):
    ...
    def _compute_debit(self, last_obs, opp_action, worker, base_env, episode,
                       env_index, coop_opp_simulated_action):
    ...
    def _compute_debit_using_rollouts(self, last_obs, opp_action, worker):
        n_steps_to_punish, policy_map, policy_agent_mapping = \
            self._prepare_to_perform_virtual_rollouts_in_env(worker)

        # Cooperative rollouts
        mean_total_reward_for_totally_coop_opp, _ = \
            self._compute_opp_mean_total_reward(worker, policy_map,
                                                policy_agent_mapping,
                                                partially_coop=False,
                                                opp_action=None,
                                                last_obs=last_obs)
        # Cooperative rollouts with first action as the real one
        mean_total_reward_for_partially_coop_opp, _ = \
            self._compute_opp_mean_total_reward(worker, policy_map,
                                                policy_agent_mapping,
                                                partially_coop=True,
    ...
    def _prepare_to_perform_virtual_rollouts_in_env(self, worker):
        ...
        policy_map = {policy_id: self
                      for policy_id in worker.policy_map.keys()}
        policy_agent_mapping = (lambda agent_id:
                                self._switch_own_and_opp(agent_id))
    ...
    def _compute_punishment_duration_from_rollouts(self, worker, last_obs):
        ...
        self.punishment_debit = self.total_debit * \
                                self.punishment_multiplier
        ...
        while continue_to_search_k:
            k_to_explore, continue_to_search_k = \
                self._search_duration_of_future_punishment(
                    k_to_explore, worker, policy_map, policy_agent_mapping,
                    last_obs)
    ...
    def _duration_found_or_continue_search(self, k_to_explore):
        ...
        if continue_to_search_k:
            k_to_explore, continue_to_search_k = \
                self._stop_search_if_all_small_k_already_explored(
                    k_to_explore)
        ...
        if continue_to_search_k and not skip_k_update:
            k_to_explore, continue_to_search_k, skip_other = \
                self._if_nothing_remains_to_explore(
                    k_to_explore)
    ...
    def _is_k_found(self, k_to_explore):
        found_k = (self.k_opp_loss[k_to_explore] >= self.punishment_debit and
    ...
    def _stop_search_if_all_small_k_already_explored(self,
                                                     k_to_explore):
        continue_to_search_k = True
        if (self.k_opp_loss[k_to_explore - 1] > self.punishment_debit
    ...
    def _compute_opp_total_reward_loss(
            ...
            last_obs):
        # Cooperative rollouts
        coop_mean_total_reward, n_steps_played = \
        ...
        partially_coop_mean_total_reward, _ = \
            self._compute_opp_mean_total_reward(
                worker, policy_map,
                policy_agent_mapping,
                partially_coop=False,
        ...
        if self.verbose > 0:
            print(f"partially_coop_mean_total_reward "
    ...
    def _compute_opp_mean_total_reward(self, worker, policy_map,
        ...
        for i in range(self.n_rollout_replicas // 2):
            ...
            coop_rollout = rollout.internal_rollout(
                ...
                num_episodes=1)
            assert coop_rollout._num_episodes == 1, \
