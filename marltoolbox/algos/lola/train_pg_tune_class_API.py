...
class LOLAPGMatrice(tune.Trainable):
    ...
    def _init_lola(self, env, seed, num_episodes, trace_length, batch_size,
        ...
        with graph.as_default() as g:
            ...
            for agent in range(self.total_n_agents):
                self.mainQN.append(
                    Qnetwork('main' + str(agent), agent, self.env, lr=lr, gamma=gamma, batch_size=batch_size,
    ...
    def step(self):
        ...
        if (self.episodes_run[agent] % self.batch_size == 0 and
            ...
            trainBatch0 = self.buffers[0].sample(self.batch_size,
                                                 self.trace_length)  # Get a random batch of experiences.
            ...
            sample_reward0 = np.reshape(trainBatch0[:, 2] - np.mean(trainBatch0[:, 2]),
                                        [-1, self.trace_length]) * self.discount
            sample_reward1 = np.reshape(trainBatch1[:, 2] - np.mean(trainBatch1[:, 2]),
                                        [-1, self.trace_length]) * self.discount
        ...
        if len(self.rList) % self.summaryLength == 0 and len(self.rList) != 0 and self.updated == True:
            ...
            print(self.total_steps, 'reward', np.mean(self.rList[-self.summaryLength:], 0) / gamma_discount, 'action',
                  (np.mean(self.aList[-self.summaryLength:], 0) * 2.0 / np.sum(
                      np.mean(self.aList[-self.summaryLength:], 0))) * 100 // 1)

            action_prob = np.mean(self.aList[-self.summaryLength:], 0) * 2.0 / np.sum(
