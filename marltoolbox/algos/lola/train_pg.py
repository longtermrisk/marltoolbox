    ...
    *,
    ...
    with tf.Session() as sess:
        ...
        for i in range(num_episodes):
            ...
            while j < max_epLength:
                ...
                for agent_role, agent in enumerate(these_agents):
                    a = sess.run(
                        ...
                        feed_dict={mainQN[agent].scalarInput: [s[agent_role]]},
                    )
                    a_all.append(a[0])
            ...
            if (
                episodes_run[agent] % batch_size == 0
                and episodes_run[agent] > 0
            ):

                # Sample from data buffer
                trainBatch0 = buffers[0].sample(
                ...
                sample_return0 = np.reshape(
                    get_monte_carlo(
                        trainBatch0[:, 2], y, trace_length, batch_size
                    ),
                    [batch_size, -1],
                )
                sample_return1 = np.reshape(
                    get_monte_carlo(
                        trainBatch1[:, 2], y, trace_length, batch_size
                    ),
                    [batch_size, -1],
                )
            ...
            if (
                ...
                print(
                    total_steps,
                    "reward",
                    np.mean(rList[-summaryLength:], 0) / gamma_discount,
                    "action",
                    (
                        np.mean(aList[-summaryLength:], 0)
                        * 2.0
                        / np.sum(np.mean(aList[-summaryLength:], 0))
                    )
                    * 100
                    // 1,
                )
                ...
                if simple_net:
                    ...
                    log_pi0, log_pi1 = sess.run(
                        [mainQN[0].log_pi, mainQN[1].log_pi],
                        feed_dict=feed_dict_log_pi,
                    )
